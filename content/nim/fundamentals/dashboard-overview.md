...
{{< call-out "note" >}}The value in the **Current Value** column reflects the disk usage of the root partition on the instance. An {{% icon triangle-alert %}} alert appears next to the hostname if the available space on the root partition drops below 20%.{{< /call-out >}}
