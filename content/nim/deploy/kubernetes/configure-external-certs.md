...
nd-product: NIMNGR
description: "Configure F5 NGINX Instance Manager to use external TLS certificates from a trusted CA instead of the self-signed certificates generated by the Helm chart."
keywords: "TLS certificates, external certificates, BYOC, bring your own certificates, cert-manager, HashiCorp Vault, PKI, Kubernetes, Helm"
nd-summary: >
  Configure F5 NGINX Instance Manager to use external TLS certificates for any or all of its services.
  Production deployments need certificates from a trusted CA instead of the self-signed certificates the Helm chart generates by default.
  You can replace individual service certificates without changing the default behavior for services you don't override.
---
