...
| **NGINX installation**       | `-n`: Installs the latest version of NGINX Open Source.<br><br>`-p`: Installs NGINX Plus as the API gateway. **Only supported when using the script in online mode.** Requires `-j`.<br><br>`-j <path>`: Path to the `license.jwt` file. Required with `-p`. |
