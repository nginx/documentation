...
{{< call-out class="tip" >}} NGINX Ingress Controller can fetch compiled bundles directly from NGINX Instance Manager or NGINX One Console without the manual download and upload steps described on this page. See [Fetch WAF bundles from remote sources]({{< ref "/nic/tutorials/bundle-sources.md" >}}) for details on automated bundle fetching with optional polling for updates. {{< /call-out >}}
...
{{< call-out class="tip" >}} You can skip this step if you intend to use an existing security policy. {{< /call-out >}}
...
{{< call-out class="warning" >}} The `content` value must be base64 encoded or you will encounter an error. {{< /call-out >}}
