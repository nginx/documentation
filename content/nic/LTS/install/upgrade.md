...
This document describes how to upgrade F5 NGINX Ingress Controller LTS when a new version releases.
...
## Minor NGINX Ingress Controller LTS upgrades

### Upgrade NGINX Ingress Controller LTS CRDs

{{< call-out "note" >}} If you are running NGINX Ingress Controller LTS v3.x, you should read [Upgrade from NGINX Ingress Controller LTS v3.x to v4.0.0]({{< ref "/nic/install/upgrade.md#upgrade-from-3x-to-4x" >}}) before continuing. {{< /call-out >}}
...
In the above command, `v{{< nic-version >}}` represents the version of the NGINX Ingress Controller LTS release rather than the Helm chart version.
...
The instructions in this section are intended only for users upgrading from NGINX Ingress Controller LTS 3.x to 4.0.0. Internal changes meant that backwards compability was not possible, requiring extra steps to upgrade.

{{< /call-out >}}

This section provides step-by-step instructions for upgrading NGINX Ingress Controller LTS from version v3.x to v4.0.0.
...
If you're using Helm chart version `v2.x`, update your `GlobalConfiguration`, `Policy`, and `TransportServer` resources from `apiVersion: k8s.nginx.org/v1alpha1` to `apiVersion: k8s.nginx.org/v1` before upgrading to NGINX Ingress Controller LTS 4.0.0.

If the Helm chart you have been using is `v1.0.2` or earlier (NGINX Ingress Controller LTS `v3.3.2`), upgrade to Helm chart `v1.4.2` (NGINX Ingress Controller LTS `v3.7.2`) before updating your GlobalConfiguration, Policy, and TransportServer resources.
...
{{< call-out "note" >}} These options apply to NGINX Ingress Controller LTS logs, and do not affect NGINX logs. {{< /call-out >}}
...
If you're using [NGINX Plus]({{< ref "/nic/overview/nginx-plus.md" >}}) with NGINX Ingress Controller LTS, you should read the [Create a license Secret]({{< ref "/nic/install/license-secret.md" >}}) topic to set up your NGINX Plus license.
...
Starting in version 3.1.0, NGINX Ingress Controller LTS uses updated Helm resource names, labels, and annotations to follow Helm best practices. [See the changes.](https://github.com/nginx/kubernetes-ingress/pull/3606)
