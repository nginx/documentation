...
This document explains how to build a F5 NGINX Ingress Controller LTS image with F5 WAF for NGINX from source code.

{{< call-out "tip" "Pre-built image alternatives" >}} 
If you'd rather not build your own NGINX Ingress Controller LTS image, see the [pre-built image options](#pre-built-images) at the end of this guide.
{{< /call-out >}}
...
Get your system ready for building and pushing the NGINX Ingress Controller LTS image with F5 WAF for NGINX.
...
Once you've successfully built the NGINX Ingress Controller LTS image with F5 WAF for NGINX, the next step is to upload it to your private Docker registry. This makes the image available for deployment to your Kubernetes cluster.
...
{{< call-out "note" >}} If you're using NGINX Ingress Controller LTS with the AppProtect WAF module and policy bundles, you will need to modify the Deployment, DaemonSet, or StatefulSet file to include volumes and volume mounts.

NGINX Ingress Controller LTS **requires** the volume mount path to be `/etc/nginx/waf/bundles`. {{< /call-out >}}
