...
{{< call-out "important" >}}
This page is only applicable to NGINX Ingress Controller LTS versions 3.2.0 - 3.7.2.

For more recent versions of NGINX Ingress Controller LTS, view the [Upgrade to NGINX Ingress Controller LTS 4.0.0]({{< ref "/nic/installation/install-nic/upgrade-to-v4.md" >}}) topic.

{{< /call-out >}}

This page describes how to enable Usage Reporting for F5 NGINX Ingress Controller LTS and how to view usage data through the API.

---

## Overview

Usage Reporting is a Kubernetes controller that connects to the NGINX Instance Manager and reports the number of NGINX Ingress Controller LTS nodes in the cluster. It is installed as a Kubernetes Deployment in the same cluster as NGINX Ingress Controller LTS whose nodes you would like reported.

To use Usage Reporting, you must have access to NGINX Instance Manager. For more information, see [NGINX Instance Manager](https://www.f5.com/products/nginx/instance-manager/). Usage Reporting is a requirement of the new Flexible Consumption Program for NGINX Ingress Controller LTS.
...
- [NGINX Ingress Controller LTS]({{< ref "/nic/" >}}) 3.2.0 - 3.7.2
...
- Access to the Kubernetes cluster where NGINX Ingress Controller LTS is deployed, with the ability to deploy a Kubernetes Deployment and a Kubernetes Secret.
...
Usage Reporting sends the number of NGINX Ingress Controller LTS instances and nodes in the cluster to NGINX Instance Manager. To view the usage data, query the NGINX Instance Manager API. The usage data is available at the following endpoint:
