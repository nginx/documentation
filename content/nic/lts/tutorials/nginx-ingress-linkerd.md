---
title: NGINX Ingress Controller LTS and Linkerd
toc: true
weight: 1800
...
- Adding Linkerd to a new NGINX Ingress Controller LTS Installation
- Adding Linkerd to an Existing NGINX Ingress Controller LTS Installation

If you are adding Linkerd to an existing installation, these are the requirements:

- A working NGINX Ingress Controller LTS instance.
- A working [Linkerd installation](https://linkerd.io/2.13/getting-started/).

## Integrating Linkerd

Linkerd integrates with NGINX Ingress Controller LTS using its control plane utility through injection.

You can do this through the use of NGINX Ingress Controller LTS's custom resource definitions (CRDs) in a Kubernetes Manifest, or Helm.
...
When installing NGINX Ingress Controller LTS with Helm, add the following annotation to your Helm deployment:
...
This annotation will instruct `helm` to tell `Linkerd` to automatically inject its sidecar during the installation of NGINX Ingress Controller LTS.

### With an existing installation

To integrate Linkerd with an existing NGINX Ingress Controller LTS installation, you will need to inject the `Linkerd` sidecar, using its `linkerd` control plane utility.
...
Once NGINX Ingress Controller LTS has been integrated with Linkerd, we can check the number of pods to confirm that the sidecar has successfully injected.
...
In the above example, `2/2` displays the number of pods, and confirms the `Linkerd` sidecar has successfully injected into NGINX Ingress Controller LTS.
...
Next, we are going to create `virtualserver` resource for NGINX Ingress Controller LTS.
...
We can now start sending traffic to NGINX Ingress Controller LTS, to verify that `Linkerd` is handling the sidecar traffic connections.
...
You can additionally view the status of NGINX Ingress Controller LTS and Linkerd by using the Viz dashboard provided by Linkerd.
