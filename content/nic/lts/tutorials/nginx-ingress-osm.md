---
title: NGINX Ingress Controller LTS and Open Service Mesh
toc: true
weight: 1800
...
There are two ways to integrate NGINX Ingress Controller LTS with Open Service Mesh (OSM):

1. Injecting an envoy sidecar directly with NGINX Ingress Controller LTS.
1. Using the Open Service Mesh `ingressBackend` "proxy" feature.

# NGINX Ingress Controller LTS and OSM with sidecar proxy injected
...
## Mark the F5 NGINX Ingress Controller LTS namespace for sidecar injection

{{< call-out "warning" >}} 

Depending on how you install NGINX Ingress Controller LTS, you might need to create the `namespace`. 

For example, if you are using Manifests to install NGINX Ingress Controller LTS, you can complete all of the steps except actually deploying NGINX Ingress Controller LTS. 
...
Next thing we need to do is install OSM into the `NGINX Ingress Controller LTS` namespace so that the `envoy` sidecar will be injected into NGINX Ingress Controller LTS.
...
# Install F5 NGINX Ingress Controller LTS
...
This annotation is *required* when injecting envoy sidecar into NGINX Ingress Controller LTS.
...
To test the integration, we will use the `httpbin` sample application from the [Ingress With Kubernetes NGINX Ingress Controller LTS](https://release-v1-2.docs.openservicemesh.io/docs/demos/ingress_k8s_nginx/) guide.
...
### Verify that the envoy sidecar has been *injected* into NGINX Ingress Controller LTS
...
2/2 shows we have two containers in the NGINX Ingress Controller LTS pod: NGINX Ingress and Envoy
...
Once OSM has been installed, this next command will mark NGINX Ingress Controller LTS as part of the OSM mesh, while also disabling sidecar injection.
...
This will generate a new client certificate (osm-nginx-client-cert) that NGINX Ingress Controller LTS will use for mTLS.
...
Now, we need to "export" out these certificates in order to use them with NGINX Ingress Controller LTS.
...
You can now send traffic through NGINX Ingress Controller LTS with open service mesh.
