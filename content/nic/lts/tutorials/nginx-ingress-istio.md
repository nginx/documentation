---
title: NGINX Ingress Controller LTS and Istio Service Mesh
toc: true
weight: 1800
...
This is accomplished using the special setting [use-cluster-ip]({{< ref "/nic/lts/configuration/virtualserver-and-virtualserverroute-resources.md#upstream" >}}) for the backend.

Here is a standard deployment of NGINX Ingress Controller LTS without a sidecar proxy injected into the pod.

{{< img src="/nic/lts/nginx_plain.png" alt="NGINX stand alone." >}}

Notice that NGINX Ingress Controller LTS enumerates the pods of the backend service and balances traffic directly to them, granting full control of the backend load balancer and stickiness behavior.
...
It is crucial to make sure you install Istio **BEFORE** installing NGINX Ingress Controller LTS. This ensures that the Istio sidecar is injected correctly into the NGINX Ingress Controller LTS pod.
...
The image below shows how an NGINX Ingress Controller LTS and Istio deployment looks:

{{< img src="/nic/lts/nginx-envoy.png" alt="NGINX with envoy sidecar." >}}

## Install NGINX Ingress Controller LTS

Once Istio is installed, you can install NGINX Ingress Controller LTS.
...
{{< img src="/nic/lts/nginx_istio_small.png" alt="NGINX Ingress pod with envoy sidecar." >}}

We can now see that after configuring Istio, an Istio sidecar proxy has been installed into the same pod as NGINX Ingress Controller LTS. Now, there are two containers in the same pod for NGINX Ingress Controller LTS: the NGINX Ingress Controller LTS container and the Istio sidecar proxy container.
...
We can see in the above output that our curl request is sent and received by NGINX Ingress Controller LTS. We can see that the envoy sidecar proxy sends the request to the service IP to the application (coffee). The full request is complete and correct. Now we have a full working NGINX+ Ingress with Istio as the sidecar proxies are deployed.

## Additional Technical information details

By default, for NGINX Ingress Controller LTS, we populate the upstream server addresses with the endpoint IPs of the pods.
...
In the 1.11 release, NGINX Ingress Controller LTS will only send one host header, depending on how you configure Ingress. By default NGINX Ingress Controller LTS will send `proxy_set_header $host`. If Ingress has been configured with `action.proxy.requestHeaders` this ensures that only one set of headers will be sent to the upstream server. In summary, by setting `action.proxy.requestHeaders` in the `VirtualServer` CRD, NGINX Ingress will only send the specified headers that have been defined.
