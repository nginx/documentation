...
This document explains how to use NGINX Ingress Controller LTS to configure NGINX Agent for sending F5 WAF for NGINX metrics to NGINX Security Monitoring.

## Before you begin

This guide assumes that you have an installation of NGINX Instance Manager with NGINX Security Monitoring which is reachable from the Kubernetes cluster on which NGINX Ingress Controller LTS is deployed.
...
## Deploying NGINX Ingress Controller LTS with NGINX Agent configuration
...
1. Add the below argument to the manifest file of NGINX Ingress Controller LTS:

    ```yaml
    args:
      - -agent=true
      - -agent-instance-group=<NGINX Ingress Controller LTS deployment name>
    ```

2. Create a ConfigMap with an `nginx-agent.conf` file which must be mounted to `/etc/nginx-agent/nginx-agent.conf` in the NGINX Ingress Controller LTS pod.
   ...
   ```yaml
    ...
    metadata:
      name: <configmap name>
      namespace: <namespace where NGINX Ingress Controller LTS will be installed>
    data:
      nginx-agent.conf: |-
        log:
          level: error
...
{{< call-out "note" >}} The `features` list must not contain `nginx-config-async` or `nginx-ssl-config` as these features can cause conflicts with NGINX Ingress Controller LTS.{{< /call-out >}}

3. Make sure that the ConfigMap is mounted to the NGINX Ingress Controller LTS pod at `/etc/nginx-agent/nginx-agent.conf` and the dynamic agent config is mounted at `/var/lib/nginx-agent` by adding the following volumes and volumeMounts to the NGINX Ingress Controller LTS deployment manifest:
...
Once NGINX Ingress Controller LTS is installed the pods will be visible in the NGINX Instance Monitoring Instances dashboard.
