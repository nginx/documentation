---
title: Customize OIDC Configuration with NGINX Ingress Controller LTS
weight: 1800
toc: true
...
The F5 NGINX Ingress Controller LTS implements OpenID Connect (OIDC) using the NGINX OpenID Connect Reference implementation: [nginx-openid-connect](https://github.com/nginxinc/nginx-openid-connect).

This guide will walk through how to customize and configure this default implementation.

## Prerequisites

{{< call-out "note" >}}This guide only works with F5 NGINX Ingress Controller LTS version 5.3.0 or above. Please make sure you are using a compatible version before proceeding.{{< /call-out >}}

This guide assumes that you have an F5 NGINX Ingress Controller LTS deployed. If not, please follow the installation steps using either the [Manifest]({{< ref "/nic/lts/install/manifests.md" >}}) or [Helm]({{< ref "/nic/lts/install/helm.md" >}}) approach.
...
2. Attach a `Volume` and `VolumeMount` to your deployment of the F5 NGINX Ingress Controller LTS
...
In the next step, NGINX Ingress Controller LTS will be deployed using this ConfigMap.

Any changes made to this ConfigMap must be made **before** deploying or updating NGINX Ingress Controller LTS. If an update is applied to the ConfigMap after NGINX Ingress Controller LTS is deployed, it will not be applied.

Applying any updates to the data in this ConfigMap will require NGINX Ingress Controller LTS to be re-deployed.

{{< /call-out >}}

## Step 3 - Add Volume and VolumeMount to the Ingress Controller deployment

In this step we will add a `Volume` and `VolumeMount` to the NGINX Ingress Controller LTS deployment.
...
Edit the NGINX Ingress Controller LTS Deployment/DaemonSet/StatefulSet yaml to include a `Volume` and `VolumeMount`.
