---
title: Using NGINX Ingress Controller LTS with NGINX Dynamic Modules
toc: true
weight: 1800
...
NGINX Plus has several Dynamic Modules that can add additional features and capabilities to NGINX, which NGINX Ingress Controller LTS can also use. To do this, you must modify your NGINX Ingress Controller LTS image to add a module, then load the updated image.
...
To build a custom NGINX Ingress Controller LTS image with specific modules, you must modify the `Dockerfile` located in the `build` directory of the code repository.

First, clone the NGINX Ingress Controller LTS repository:
...
In this example, we add the `Headers-more` dynamic module to the NGINX Ingress Controller LTS image. We choose the `debian-plus` operating system: modify the entry for the system you are using.
...
In the snippet above there is a line similar to the following, which you must modify to add a dynamic module to NGINX Ingress Controller LTS.
...
Once the new NGINX Ingress module image has built successfully, the next step is to load the module into NGINX Ingress Controller LTS when it is deployed into your Kubernetes cluster.

To do this, modify your NGINX Ingress Controller LTS configuration to add the module into the `main` context, which can be done through both Manifest and Helm deployments.
