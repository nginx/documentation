...
This document explains how to create and use a license secret for F5 NGINX Ingress Controller LTS.
...
The JWT needs to be configured before deploying NGINX Ingress Controller LTS. 

It must be stored in a Kubernetes Secret of type `nginx.com/license` in the same namespace as your NGINX Ingress Controller LTS pod(s).
...
If you've already deployed NGINX Ingress Controller LTS and need to rotate or renew the JWT (for example, when the existing token is about to expire or has been replaced), update the existing Secret in place.
...
If using a name other than the default `license-token`, provide the name of this Secret when installing NGINX Ingress Controller LTS:
...
If you are deploying NGINX Ingress Controller LTS in an "air-gapped" environment you will need to report to [NGINX Instance Manager]({{< ref "/nim/" >}}) instead of the default licensing endpoint.
...
To use Client Auth with NGINX Instance Manager, first create a Secret of type `kubernetes.io/tls` in the same namespace as the NGINX Ingress Controller LTS pods.
