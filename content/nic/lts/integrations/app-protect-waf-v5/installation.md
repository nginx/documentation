---
title: Build NGINX Ingress Controller LTS with F5 WAF for NGINX
weight: 100
toc: true
...
This document explains how to build a F5 NGINX Ingress Controller LTS image with F5 WAF for NGINX v5 from source code.
...
Get your system ready for building and pushing the NGINX Ingress Controller LTS image with F5 WAF for NGINX v5.
...
1. Clone the NGINX Ingress Controller LTS repository:
...
Once you've successfully pulled the WAF v5 manager and enforcer images and built the NGINX Ingress Controller LTS image with F5 WAF for NGINX v5, the next step is to upload them to your private Docker registry. This makes the image available for deployment to your Kubernetes cluster.
...
{{< call-out "important" >}} NGINX Ingress Controller LTS with the AppProtect WAF v5 module works only with policy bundles. You need to modify the Deployment, DaemonSet, or StatefulSet file to include volumes, volume mounts and two WAF 5 docker images: `waf-config-mgr` and `waf-enforcer`.

NGINX Ingress Controller LTS **requires** the volume mount path to be `/etc/app_protect/bundles`. {{< /call-out >}}
...
This ensures that both the `waf-enforcer` and `waf-config-mgr` containers are deployed alongside the NGINX Ingress Controller LTS containers.
...
You have two options for deploying NGINX Ingress Controller LTS:
