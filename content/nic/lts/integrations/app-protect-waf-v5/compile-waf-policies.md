...
This guide describes how to use F5 NGINX Instance Manager to compile F5 WAF for NGINX Policies for use with NGINX Ingress Controller LTS.

F5 WAF for NGINX uses policies to configure which security features are set. When these policies are changed, they need to be compiled so that the engine can begin to use them. Compiling policies can take a large amount of time and resources. You can do this with the NGINX Instance Manager. This reduces the impact on a NGINX Ingress Controller LTS deployment.

By using NGINX Instance Manager to compile WAF policies, the policy bundle can also be used immediately by NGINX Ingress Controller LTS without reloading.

The following steps describe how to use the NGINX Instance Manager API to create a new security policy, compile a bundle, then add it to NGINX Ingress Controller LTS.
...
- A NGINX Ingress Controller LTS [deployment with F5 WAF for NGINX]({{< ref "/nic/lts/integrations/app-protect-waf-v5/installation.md" >}}).
...
To use WAF security bundles, your NGINX Ingress Controller LTS instance must have *volumes* and *volumeMounts*. Precise paths are used to detect when bundles are uploaded to the cluster.
...
Upload the security log bundle binary file to the NGINX Ingress Controller LTS pods.
...
Upload the binary file to the NGINX Ingress Controller LTS pods.
...
To process a bundle, you must create a new WAF policy. This policy is added to `/etc/app_protect/bundles`, allowing NGINX Ingress Controller LTS to load it into WAF.
