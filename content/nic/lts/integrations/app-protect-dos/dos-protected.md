...
If a resource passes structural validation, then NGINX Ingress Controller LTS will start comprehensive validation.

#### Comprehensive validation

NGINX Ingress Controller LTS validates the fields of a DoS protected resource. If a resource is invalid, NGINX Ingress Controller LTS will reject it. The resource will continue to exist in the cluster, but NGINX Ingress Controller LTS will ignore it.

You can use `kubectl` to check if NGINX Ingress Controller LTS successfully applied a DoS protected resource configuration. For our example `dos-protected` DoS protected resource, we can run:
...
If you create an invalid resource, NGINX Ingress Controller LTS will reject it and emit a Rejected event. For example, if you create a dos protected resource `dos-protected` with an invalid URI `bad` in the `dosSecurityLog/dosLogDest` field, you will get:
...
{{< call-out "warning" >}} If you invalidate an existing resource, NGINX Ingress Controller LTS will reject it. {{< /call-out >}}
