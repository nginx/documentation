...
This document shows a basic Ingress resource definition for F5 NGINX Ingress Controller LTS. It load balances requests for two services as part of a single application.
...
{{< call-out "note" >}} For complete instructions on deploying Ingress and Secret resources in the cluster, see the [complete example](https://github.com/nginx/kubernetes-ingress/tree/v{{< nic-lts-version >}}/examples/ingress-resources/complete-example) in the GitHub repository. {{< /call-out >}}
...
NGINX Ingress Controller LTS generates NGINX configuration by executing a template file that contains the configuration options.

These options are set with the Ingress resource and NGINX Ingress Controller LTS's ConfigMap.
