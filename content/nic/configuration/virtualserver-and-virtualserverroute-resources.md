...
|``client-body-buffer-size`` | Sets the size of the buffer used for reading the client request body. See the [client-body-buffer-size](https://nginx.org/en/docs/http/ngx_http_core_module.html#client_body_buffer_size) directive. The default is set in the ``client-body-buffer-size`` ConfigMap key. | ``string`` | No |
