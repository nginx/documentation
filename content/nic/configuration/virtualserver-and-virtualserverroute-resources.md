...
product: INGRESS
id: DOCS-599
canonical-url: https://docs.nginx.com/nic/configuration/virtualserver-and-virtualserverroute-resources/
---
...
They enable use cases not supported with the Ingress resource, such as traffic splitting and advanced content-based routing. You implement the resources as [Custom Resources](https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-resources/).

The GitHub repository has [examples of the resources](https://github.com/nginx/kubernetes-ingress/tree/v
5.4.1
/examples/custom-resources) for specific use cases.
...
You can connect a VirtualServer to VirtualServerRoute resources in two ways:

- Reference a specific VirtualServerRoute by name (and optional namespace) using `route`.
- Select VirtualServerRoute resources dynamically using `routeSelector` and Kubernetes labels.

The example below shows a VirtualServer with both traditional static routes and a dynamic route using `routeSelector`. The route with path `/dynamic` will automatically include any VirtualServerRoute resources that have the label `app: cafe`:
...
| ---| ---| ---| --- |
|``host`` | The host (domain name) of the server. Must be a valid subdomain as defined in RFC 1123, such as ``my-app`` or ``hello.example.com``. When using a wildcard domain like ``*.example.com`` enclose the domain in double quotes. The ``host`` value needs to be unique among all Ingress and VirtualServer resources. See also [Handling Host and Listener Collisions](https://docs.nginx.com/nic/configuration/host-and-listener-collisions). | ``string`` | Yes |
...
|Field | Description | Type | Required |
| ---| ---| ---| --- |
|``secret`` | The name of a secret with a TLS certificate and key. The secret must belong to the same namespace as the VirtualServer. The secret must be of the type ``kubernetes.io/tls`` and contain keys named ``tls.crt`` and ``tls.key`` that contain the certificate and private key as described [here](https://kubernetes.io/docs/concepts/services-networking/ingress/#tls). If the secret doesn't exist or is invalid, NGINX will break any attempt to establish a TLS connection to the host of the VirtualServer. If you do not specify the secret but configure a [wildcard TLS secret](https://docs.nginx.com/nic/configuration/global-configuration/command-line-arguments#cmdoption-wildcard-tls-secret), NGINX will use the wildcard secret for TLS termination. | ``string`` | No |
...
|``issuer-kind`` | The kind of the external issuer resource, for example AWSPCAIssuer. This is only necessary for out-of-tree issuers. Do not define this if `cluster-issuer` is also defined. | ``string`` | No |
|``issuer-group`` | The API group of the external issuer controller, for example awspca.cert-manager.io. This is only necessary for out-of-tree issuers. Do not define this if `cluster-issuer` is also defined. | ``string`` | No |
|``common-name`` | This field allows you to configure spec.commonName for the Certificate. This configuration adds a CN to the x509 certificate. | ``string`` | No |
|``duration`` | This field allows you to configure spec.duration for the Certificate. Specify it using a [Go time.Duration](https://pkg.go.dev/time#ParseDuration) string format, which does not allow the d (days) suffix. You must specify these values using s, m, and h suffixes instead. | ``string`` | No |
|``renew-before`` |  this annotation allows you to configure spec.renewBefore for the Certificate. Specify it using a [Go time.Duration](https://pkg.go.dev/time#ParseDuration) string format, which does not allow the d (days) suffix. You must specify these values using s, m, and h suffixes instead. | ``string`` | No |
|``usages`` |  This field allows you to configure spec.usages for the Certificate. Pass a string with comma-separated values i.e. ``key agreement,digital signature, server auth``. See the exhaustive list of supported key usages in the [the cert-manager api documentation](https://cert-manager.io/docs/reference/api-docs/#cert-manager.io/v1.KeyUsage). | ``string`` | No |
...
The respective listeners used must reference the name of a listener defined using a [GlobalConfiguration](https://docs.nginx.com/nic/configuration/global-configuration/globalconfiguration-resource) resource.
For example:
...
|Field | Description | Type | Required |
| ---| ---| ---| --- |
|``http`` |  The name of am HTTP listener defined in a [GlobalConfiguration](https://docs.nginx.com/nic/configuration/global-configuration/globalconfiguration-resource) resource. | ``string`` | No |
|``https`` |  The name of an HTTPS listener defined in a [GlobalConfiguration](https://docs.nginx.com/nic/configuration/global-configuration/globalconfiguration-resource) resource. | ``string`` | No |
...
|``labels`` | Configure labels for the Endpoint resources that ExternalDNS consumes. | ``map[string]string`` | No |
...
|``recordType`` | The record Type to create, e.g. "A", "AAAA", "CNAME". NGINX automatically computes it based on the external endpoints if you do not define it. | ``string`` | No |
...
The policy field references a [Policy resource](https://docs.nginx.com/nic/configuration/policy-resource) by its name and optional namespace. For example:
...
|``namespace`` | The namespace of a policy. If you do not specify it, use the namespace of the VirtualServer resource. | ``string`` | No |
...
|Field | Description | Type | Required |
| ---| ---| ---| --- |
|``path`` | The path of the route. NGINX will match it against the URI of a request. Possible values are: a prefix ( ``/`` , ``/path`` ), an exact match ( ``=/exact/match`` ), a case insensitive regular expression ( ``~*^/Bar.*\.jpg`` ) or a case sensitive regular expression ( ``~^/foo.*\.jpg`` ). In the case of a prefix (must start with ``/`` ) or an exact match (must start with ``=`` ), the path must not include any whitespace characters, ``{`` , ``}`` or ``;``. In the case of the regex matches, escape all double quotes ``"`` and do not end the match with an unescaped backslash ``\``. The path must be unique among the paths of all routes of the VirtualServer. Check the [location](https://nginx.org/en/docs/http/ngx_http_core_module.html#location) directive for more information. | ``string`` | Yes |
|``policies`` | A list of policies. The policies override the policies of the same type defined in the ``spec`` of the VirtualServer. See [Applying Policies](https://docs.nginx.com/nic/configuration/policy-resource#applying-policies) for more details. | [[]policy](#virtualserverpolicy) | No |
...
|``matches`` | The matching rules for advanced content-based routing. Requires the default ``action`` or ``splits``.  The default ``action`` or ``splits`` handles unmatched requests. | [matches](#match) | No |
...
|``routeSelector`` | Selects VirtualServerRoute resources using Kubernetes label selectors instead of specifying them by name and namespace. Use this to add or change VirtualServerRoute resources without updating the VirtualServer, as long as labels and paths match. | [routeSelector](#virtualserverrouterouteselector) | No |
...
The RouteSelector field allows you to select VirtualServerRoute resources using Kubernetes labels instead of specifying them by name and namespace.

This lets you add, update, or move VirtualServerRoute resources without changing the VirtualServer, as long as:

- The VirtualServerRoute uses labels that match the `routeSelector`.
- The VirtualServerRoute `host` matches the VirtualServer `host`.
- The VirtualServerRoute `subroutes[].path` values follow the VirtualServer route `path` rules (see [VirtualServerRoute.Subroute](#virtualserverroutesubroute)).
...
|Field | Description | Type | Required |
| ---| ---| ---| --- |
|``matchLabels`` | A map of key-value pairs. Each key-value pair in the matchLabels map is equivalent to an element of matchExpressions, whose key field is "key", the operator is "In", and the values array contains only "value". AND the requirements together. | ``map[string]string`` | Yes |
|``matchExpressions`` | A list of label selector requirements. AND the requirements together. For more information on label selector requirements, see the [Kubernetes documentation](https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/#resources-that-support-set-based-requirements). | N/A | No |

## VirtualServerRoute specification

The VirtualServerRoute resource defines a route for a VirtualServer. It can consist of one or multiple subroutes. The VirtualServerRoute is an alternative to [Mergeable Ingress types](https://docs.nginx.com/nic/configuration/ingress-resources/cross-namespace-configuration).
...
Note that each subroute must have a `path` that starts with the same prefix (here `/coffee`), which the route of the VirtualServer defines. Additionally, the `host` in the VirtualServerRoute must be the same as the `host` of the VirtualServer.

|Field | Description | Type | Required |
| ---| ---| ---| --- |
|``host`` | The host (domain name) of the server. Must be a valid subdomain as defined in RFC 1123, such as ``my-app`` or ``hello.example.com``. When using a wildcard domain like ``*.example.com`` enclose the domain in double quotes. Must be the same as the ``host`` of the VirtualServer that references this resource. | ``string`` | Yes |
|``upstreams`` | A list of upstreams. | [[]upstream](#upstream) | No |
...
|Field | Description | Type | Required |
| ---| ---| ---| --- |
|``path`` | The path of the subroute. NGINX will match it against the URI of a request. Possible values are: a prefix ( ``/`` , ``/path`` ), an exact match ( ``=/exact/match`` ), a case insensitive regular expression ( ``~*^/Bar.*\\.jpg`` ) or a case sensitive regular expression ( ``~^/foo.*\\.jpg`` ). In the case of a prefix, the path must start with the same path as the path of the route of the VirtualServer that references this resource. In the case of an exact or regex match, the path must be the same as the path of the route of the VirtualServer that references this resource. NGINX does not accept a matching path of the route of the VirtualServer but in different type, e.g. a regex path (`~/match`) cannot work with a prefix path in VirtualServer (`/match`) In the case of a prefix or an exact match, the path must not include any whitespace characters, ``{`` , ``}`` or ``;``.  In the case of the regex matches, escape all double quotes ``\\\"`` and do not end the match with an unescaped backslash ``\\\\``. The path must be unique among the paths of all subroutes of the VirtualServerRoute. | ``string`` | Yes |\n
|``policies`` | A list of policies. The policies override *all* policies defined in the route of the VirtualServer that references this resource. The policies also override the policies of the same type defined in the ``spec`` of the VirtualServer. See [Applying Policies](https://docs.nginx.com/nic/configuration/policy-resource#applying-policies) for more details. | [[]policy](#virtualserverpolicy) | No |
|``action`` | The default action to perform for a request. | [action](#action) | No |
...
|``service`` | The name of a [service](https://kubernetes.io/docs/concepts/services-networking/service/). If the Service belongs to a different namespace than the VirtualServer or VirtualServerRoute, you need to include the namespace. For example, ``tea-namespace/tea``. If the service doesn't exist, NGINX will assume the service has zero endpoints and return a ``502`` response for requests for this upstream. For NGINX Plus only, services of type [ExternalName](https://kubernetes.io/docs/concepts/services-networking/service/#externalname) are also supported in the same namespace (check the [prerequisites](https://github.com/nginx/kubernetes-ingress/tree/v
5.4.1
/examples/ingress-resources/externalname-services#prerequisites) ). | ``string`` | Yes |
|``subselector`` | Selects the pods within the service using label keys and values. By default, it selects all pods of the service. Note: expect the specified labels to be present in the pods when you create them. If you update the pod labels, NGINX Ingress Controller will not see that change until you change the number of the pods. | ``map[string]string`` | No |
|``use-cluster-ip`` | Enables using the Cluster IP and port of the service instead of the default behavior of using the IP and port of the pods. When you enable this field, the fields that configure NGINX behavior related to multiple upstream servers (like ``lb-method`` and ``next-upstream``) will have no effect, as NGINX Ingress Controller will configure NGINX with only one upstream server that will match the service Cluster IP. | ``boolean`` | No |
...
|``lb-method`` | The load [balancing method](https://docs.nginx.com/nginx/admin-guide/load-balancer/http-load-balancer#choosing-a-load-balancing-method). To use the round-robin method, specify ``round_robin``. The ``lb-method`` ConfigMap key specifies the default. | ``string`` | No |
|``fail-timeout`` | The time during which the specified number of unsuccessful attempts to communicate with an upstream server should happen to consider the server unavailable. See the [fail_timeout](https://nginx.org/en/docs/http/ngx_http_upstream_module.html#fail_timeout) parameter of the server directive. The ``fail-timeout`` ConfigMap key sets the default. | ``string`` | No |
|``max-fails`` | The number of unsuccessful attempts to communicate with an upstream server that should happen in the duration set by the ``fail-timeout`` to consider the server unavailable. See the [max_fails](https://nginx.org/en/docs/http/ngx_http_upstream_module.html#max_fails) parameter of the server directive. The ``max-fails`` ConfigMap key sets the default. | ``int`` | No |
|``max-conns`` | The maximum number of simultaneous active connections to an upstream server. See the [max_conns](https://nginx.org/en/docs/http/ngx_http_upstream_module.html#max_conns) parameter of the server directive. By default there is no limit. Note: if you enable keepalive connections, the total number of active and idle keepalive connections to an upstream server may exceed the ``max_conns`` value. | ``int`` | No |
|``keepalive`` | Configures the cache for connections to upstream servers. The value ``0`` disables the cache. See the [keepalive](https://nginx.org/en/docs/http/ngx_http_upstream_module.html#keepalive) directive. The ``keepalive`` ConfigMap key sets the default. | ``int`` | No |
|``connect-timeout`` | The timeout for establishing a connection with an upstream server. See the [proxy_connect_timeout](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_connect_timeout) directive. The ``proxy-connect-timeout`` ConfigMap key specifies the default. | ``string`` | No |
|``read-timeout`` | The timeout for reading a response from an upstream server. See the [proxy_read_timeout](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_read_timeout) directive.  The ``proxy-read-timeout`` ConfigMap key specifies the default. | ``string`` | No |
|``send-timeout`` | The timeout for transmitting a request to an upstream server. See the [proxy_send_timeout](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_send_timeout) directive. The ``proxy-send-timeout`` ConfigMap key specifies the default. | ``string`` | No |
|``next-upstream`` | Specifies in which cases to pass a request to the next upstream server. See the [proxy_next_upstream](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_next_upstream) directive. The default is ``error timeout``. | ``string`` | No |
|``next-upstream-timeout`` | The time during which you can pass a request to the next upstream server. See the [proxy_next_upstream_timeout](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_next_upstream_timeout) directive. The ``0`` value turns off the time limit. The default is ``0``. | ``string`` | No |
...
|``client-max-body-size`` | Sets the maximum allowed size of the client request body. See the [client_max_body_size](https://nginx.org/en/docs/http/ngx_http_core_module.html#client_max_body_size) directive. The ``client-max-body-size`` ConfigMap key sets the default. | ``string`` | No |
|``client-body-buffer-size`` | Sets the value of the [client_body_buffer_size](https://nginx.org/en/docs/http/ngx_http_core_module.html#client_body_buffer_size) directive. The ``client-body-buffer-size`` ConfigMap key sets the default. | ``string`` | No |
...
|``slow-start`` | The slow start allows an upstream server to gradually recover its weight from 0 to its nominal value after it recovers or becomes available, or when the server becomes available after a period of time when NGINX considered it unavailable. By default, NGINX disables slow start. See the [slow_start](https://nginx.org/en/docs/http/ngx_http_upstream_module.html#slow_start) parameter of the server directive. Note: You cannot use the parameter along with the ``random`` , ``hash`` or ``ip_hash`` load balancing methods and NGINX will ignore it. | ``string`` | No |
|``queue`` | Configures a queue for an upstream. Place a client request into the queue if an upstream server is not available for selection immediately while processing the request. By default, NGINX configures no queue. Note: this feature is supported only in NGINX Plus. | [queue](#upstreamqueue) | No |
|``buffering`` | Enables buffering of responses from the upstream server. See the [proxy_buffering](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_buffering) directive. The ``proxy-buffering`` ConfigMap key sets the default. | ``boolean`` | No |
...
|``buffer-size`` | Sets the size of the buffer used for reading the first part of a response received from the upstream server. See the [proxy_buffer_size](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_buffer_size) directive. The ``proxy-buffer-size`` ConfigMap key sets the default. | ``string`` | No |
|``busy-buffers-size`` | Sets the size of the buffer used for reading a response from the upstream server when the response is larger than the ``buffer-size``. See the [proxy_busy_buffers_size](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_busy_buffers_size) directive. The ``proxy-busy-buffers-size`` ConfigMap key sets the default. | ``string`` | No |
...
|``type`` |The type of the upstream. Supported values are ``http`` and ``grpc``. The default is ``http``. For gRPC, it is necessary to enable HTTP/2 in the [ConfigMap](https://docs.nginx.com/nic/configuration/global-configuration/configmap-resource#listeners) and configure TLS termination in the VirtualServer. | ``string`` | No |
|``backup`` | The name of the backup service of type [ExternalName](https://kubernetes.io/docs/concepts/services-networking/service/#externalname). NGINX uses this when the primary servers are unavailable. Note: You cannot use the parameter along with the ``random`` , ``hash`` or ``ip_hash`` load balancing methods. | ``string`` | No |
|``backupPort`` | The port of the backup service. Specify the backup port if you provide the backup service name. The port must fall into the range ``1..65535``. | ``uint16`` | No |
...
|Field | Description | Type | Required |
| ---| ---| ---| --- |
|``number`` | Configures the number of buffers. The ``proxy-buffers`` ConfigMap key sets the default. | ``int`` | Yes |
|``size`` | Configures the size of a buffer. The ``proxy-buffers`` ConfigMap key sets the default. | ``string`` | Yes |

### Upstream.TLS

|Field | Description | Type | Required |
| ---| ---| ---| --- |
|``enable`` | Enables HTTPS for requests to upstream servers. The default is ``False`` , meaning that NGINX uses HTTP. Note: by default, NGINX will not verify the upstream server certificate. To enable the verification, configure an [EgressMTLS Policy](https://docs.nginx.com/nic/configuration/policy-resource/#egressmtls). | ``boolean`` | No |

### Upstream.Queue

The queue field configures a queue. Place a client request into the queue if NGINX cannot select an upstream server immediately while processing the request:
...
|``timeout`` | The timeout of the queue. Do not queue a request for longer than the timeout. The default is ``60s``. | ``string`` | No |
...
```
 ...
 This feature is supported only in NGINX Plus. 
...
|``fails`` | The number of consecutive failed health checks of a particular upstream server after which NGINX will consider this server unhealthy. The default is ``1``. | ``integer`` | No |
|``passes`` | The number of consecutive passed health checks of a particular upstream server after which NGINX will consider the server healthy. The default is ``1``. | ``integer`` | No |
|``port`` | The port for health check requests. By default, use the [server port](https://nginx.org/en/docs/http/ngx_http_upstream_hc_module.html#health_check_port). Note: in contrast with the port of the upstream, this port is not a service port, but a port of a pod. | ``integer`` | No |
|``tls`` | The TLS configuration for health check requests. By default, use the ``tls`` field of the upstream. | [upstream.tls](#upstreamtls) | No |
|``connect-timeout`` | The timeout for establishing a connection with an upstream server. By default, use the ``connect-timeout`` of the upstream. | ``string`` | No |
|``read-timeout`` | The timeout for reading a response from an upstream server. By default, use the ``read-timeout`` of the upstream. | ``string`` | No |
|``send-timeout`` | The timeout for transmitting a request to an upstream server. By default, use the ``send-timeout`` of the upstream. | ``string`` | No |
...
|``grpcService`` | The gRPC service that the upstream server monitors. Only valid on gRPC type upstreams. | ``string`` | No |
|``mandatory`` | Require every newly added server to pass all configured health checks before NGINX Plus sends traffic to it. If you do not specify this, or set it to false, the server will be initially considered healthy. When combined with [slow-start](https://nginx.org/en/docs/http/ngx_http_upstream_module.html#slow_start), it gives a new server more time to connect to databases and “warm up” before it handles its full share of traffic. | ``bool`` | No |
|``persistent`` | Set the initial “up” state for a server after reload if NGINX considered the server healthy before reload. Enabling persistent requires that the mandatory parameter is also set to `true`. | ``bool`` | No |
|``keepalive-time`` | Enables [keepalive](https://nginx.org/en/docs/http/ngx_http_upstream_module.html#keepalive) connections for health checks and specifies the time during which a single keepalive connection can process requests. The default is ``60s``. | ``string`` | No |

### Upstream.SessionCookie

The SessionCookie field configures session persistence which allows requests from the same client to go to the same upstream server. NGINX passes the information about the designated upstream server in a session cookie.
...
See the [`sticky`](https://nginx.org/en/docs/http/ngx_http_upstream_module.html?#sticky) directive for additional information. The session cookie corresponds to the `sticky cookie` method.
 ...
 Session persistence via sticky cookie is available in both NGINX (since version 1.29.6) and NGINX Plus. 
...
|``path`` | The path for which NGINX sets the cookie. | ``string`` | No |
|``expires`` | The time for which a browser should keep the cookie. Set it to the special value ``max`` , which will cause the cookie to expire on ``31 Dec 2037 23:55:55 GMT``. | ``string`` | No |
|``domain`` | The domain for which NGINX sets the cookie. | ``string`` | No |
...
In the example below, client requests go to an upstream `coffee`:
...
|Field | Description | Type | Required |
| ---| ---| ---| --- |
|``pass`` | Passes requests to an upstream. Define the upstream with that name in the resource. | ``string`` | No |
...
In the example below, client requests go to a url `http://www.nginx.com`:
...
|Field | Description | Type | Required |
| ---| ---| ---| --- |
|``url`` | The URL to redirect the request to. Supported NGINX variables: ``$scheme`` , ``$http_x_forwarded_proto`` , ``$request_uri`` , ``$host``. Enclose variables in curly braces. For example: ``${host}${request_uri}``. | ``string`` | Yes |
|``code`` | The status code of a redirect. The allowed values are: ``301`` , ``302`` , ``307`` , ``308``. The default is ``301``. | ``int`` | No |
...
|``body`` | The body of the response. Supports NGINX variables*. Enclose variables in curly brackets. For example: ``Request is ${request_uri}\n``. | ``string`` | Yes |
...
In the example below, NGINX rewrites the request URI to `/`, and modifies the request and the response headers:
...
|Field | Description | Type | Required |
| ---| ---| ---| --- |
|``upstream`` | The name of the upstream to which NGINX will proxy the requests. Define the upstream with that name in the resource. | ``string`` | Yes |
...
|``rewritePath`` | The rewritten URI. If the route path is a regular expression -- starts with `~` -- the `rewritePath` can include capture groups with ``$1-9``. For example `$1` for the first group, and so on. For more information, check the [rewrite](https://github.com/nginx/kubernetes-ingress/tree/v
5.4.1
/examples/custom-resources/rewrites) example. | ``string`` | No |
...
| ---| ---| ---| --- |
|``name`` | The name of the header. | ``string`` | Yes |
|``value`` | The value of the header. Supports NGINX variables*. Enclose variables in curly brackets. For example: ``${scheme}``. | ``string`` | No |
...
|Field | Description | Type | Required |
| ---| ---| ---| --- |
|``hide`` | The headers that the client will not receive* in the response from a proxied upstream server. See the [proxy_hide_header](http://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_hide_header) directive for more information. | ``[]string`` | No |
...
\** -- You can ignore the following fields: `X-Accel-Redirect`, `X-Accel-Expires`, `X-Accel-Limit-Rate`, `X-Accel-Buffering`, `X-Accel-Charset`, `Expires`, `Cache-Control`, `Set-Cookie` and `Vary`.
...
\* -- Supported NGINX variables: `$request_uri`, `$request_method`, `$request_body`, `$scheme`, `$http_`, `$args`, `$arg_`, `$cookie_`, `$host`, `$request_time`, `$request_length`, `$nginx_version`, `$pid`, `$connection`, `$remote_addr`, `$remote_port`, `$time_iso8601`, `$time_local`, `$server_addr`, `$server_port`, `$server_name`, `$server_protocol`, `$connections_active`, `$connections_reading`, `$connections_writing`, `$connections_waiting`, `$ssl_cipher`, `$ssl_ciphers`, `$ssl_client_cert`, `$ssl_client_escaped_cert`, `$ssl_client_fingerprint`, `$ssl_client_i_dn`, `$ssl_client_i_dn_legacy`, `$ssl_client_raw_cert`, `$ssl_client_s_dn`, `$ssl_client_s_dn_legacy`, `$ssl_client_serial`, `$ssl_client_v_end`, `$ssl_client_v_remain`, `$ssl_client_v_start`, `$ssl_client_verify`, `$ssl_curves`, `$ssl_early_data`, `$ssl_protocol`, `$ssl_server_name`, `$ssl_session_id`, `$ssl_session_reused`, `$jwt_claim_` (NGINX Plus only) and `$jwt_header_` (NGINX Plus only).
 ...
 If `always` is false, add the response header only if the response status code is any of `200`, `201`, `204`, `206`, `301`, `302`, `303`, `304`, `307` or `308`. 
...
|``value`` | The value to match the condition against. The section below the table shows how to define a value. | ``string`` | Yes |
...
- *Matching with a regular expression*. Note that NGINX supports regular expressions compatible with those used by the Perl programming language (PCRE). For example:
 ...
 A value must not include any unescaped double quotes (`"`) and must not end with an unescaped backslash (`\`). For example, the following are invalid values: `some"value`, `somevalue\`. 
...
| ---| ---| ---| --- |
|``code`` | The status code of a redirect. The allowed values are: ``301`` , ``302`` , ``307`` , ``308``.  The default is ``301``. | ``int`` | No |
|``url`` | The URL to redirect the request to. Supported NGINX variables: ``$scheme`` and ``$http_x_forwarded_proto``. Enclose variables in curly braces. For example: ``${scheme}``. | ``string`` | Yes |
...
|``body`` | The body of the response. Supported NGINX variable: ``$upstream_status`` . Enclose variables in curly braces. For example: ``${upstream_status}``. | ``string`` | Yes |
...
| ---| ---| ---| --- |
|``name`` | The name of the header. | ``string`` | Yes |
|``value`` | The value of the header. Supported NGINX variable: ``$upstream_status`` . Enclose variables in curly braces. For example: ``${upstream_status}``. | ``string`` | No |
...
For additional information, view the [Advanced configuration with Snippets](https://docs.nginx.com/nic/configuration/ingress-resources/advanced-configuration-with-snippets) topic.
...
You can customize the NGINX configuration for VirtualServer and VirtualServerRoutes resources using the [ConfigMap](https://docs.nginx.com/nic/configuration/global-configuration/configmap-resource). Most of the ConfigMap keys are supported, with the following exceptions:
