...
The policy field references a [Policy resource]({{< ref "/nic/configuration/policy-resource/before-you-begin.md" >}}) by its name and optional namespace. For example:
...
|``policies`` | A list of policies. The policies override the policies of the same type defined in the ``spec`` of the VirtualServer. See [Applying policies]({{< ref "/nic/configuration/policy-resource/applying-policies.md" >}}) for more details. | [[]policy](#virtualserverpolicy) | No |
...
|``add-header-inherit`` | Controls header inheritance behavior at the location level. Allowed values are ``on``, ``off``, and ``merge``. | ``string`` | No |
...
|``proxy-http-version`` | The HTTP protocol version for connections to the upstream servers. Sets the [proxy_http_version](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_http_version) directive. Supported values are ``1.0``, ``1.1``, and ``2``. If this field is not set and the backend Service port defines ``appProtocol: kubernetes.io/h2c``, NGINX Ingress Controller uses HTTP/2 (``2``) automatically. An explicit value takes precedence over the value inferred from ``appProtocol``. When this field is unset and no ``appProtocol`` matches, NGINX uses its default. The setting is ignored for upstreams with ``type: grpc``, which always use HTTP/2. Note: the value ``2`` requires NGINX >= v1.29.4. | ``string`` | No |
...
{{< call-out class="note" >}} This feature is supported only in NGINX Plus. {{< /call-out >}}
...
{{< call-out class="note" >}} Session persistence via sticky cookie is available in both NGINX (since version 1.29.6) and NGINX Plus. {{< /call-out >}}
...
{{< call-out class="note" >}} If `always` is false, the response header is added only if the response status code is any of `200`, `201`, `204`, `206`, `301`, `302`, `303`, `304`, `307` or `308`. {{< /call-out >}}
...
{{< call-out class="note" >}} A match must include exactly one of the following: `action` or `splits`. {{< /call-out >}}
...
{{< call-out class="note" >}}  a condition must include exactly one of the following: `header`, `cookie`, `argument` or `variable`. {{< /call-out >}}
...
{{< call-out class="note" >}} A value must not include any unescaped double quotes (`"`) and must not end with an unescaped backslash (`\`). For example, the following are invalid values: `some"value`, `somevalue\`. {{< /call-out >}}
...
{{< call-out class="note" >}} An errorPage must include exactly one of the following: `return` or `redirect`. {{< /call-out >}}
