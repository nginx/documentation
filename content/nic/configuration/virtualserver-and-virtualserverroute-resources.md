...
|``issuer-kind`` | The kind of the external issuer resource, for example AWSPCAIssuer. This is only necessary for out-of-tree issuers. You cannot define this if `cluster-issuer` is also defined. | ``string`` | No |
|``issuer-group`` | The API group of the external issuer controller, for example awspca.cert-manager.io. This is only necessary for out-of-tree issuers. You cannot define this if `cluster-issuer` is also defined. | ``string`` | No |
...
|``duration`` | This field allows you to configure spec.duration field for the Certificate that cert-manager generates. Specify a [Go time.Duration](https://pkg.go.dev/time#ParseDuration) string format, which does not allow the d (days) suffix. Use s, m, and h suffixes instead. | ``string`` | No |
|``renew-before`` |  this annotation allows you to configure spec.renewBefore field for the Certificate that cert-manager generates. Specify a [Go time.Duration](https://pkg.go.dev/time#ParseDuration) string format, which does not allow the d (days) suffix. Use s, m, and h suffixes instead. | ``string`` | No |
|``usages`` |  This field allows you to configure spec.usages field for the Certificate that cert-manager generates. Pass a string with comma-separated values i.e. ``key agreement,digital signature, server auth``. See the [the cert-manager api documentation](https://cert-manager.io/docs/reference/api-docs/#cert-manager.io/v1.KeyUsage) for an exhaustive list of supported key usages. | ``string`` | No |
...
|``recordType`` | The record Type to create, e.g. "A", "AAAA", "CNAME". NGINX Ingress Controller automatically computes this based on the external endpoints if you do not define it. | ``string`` | No |
...
|Field | Description | Type | Required |
| ---| ---| ---| --- |
|``path`` | The path of the route. NGINX will match it against the URI of a request. Possible values are: a prefix ( ``/`` , ``/path`` ), an exact match ( ``=/exact/match`` ), a case insensitive regular expression ( ``~*^/Bar.*\\.jpg`` ) or a case sensitive regular expression ( ``~^/foo.*\\.jpg`` ). In the case of a prefix (must start with ``/`` ) or an exact match (must start with ``=`` ), the path must not include any whitespace characters, ``{`` , ``}`` or ``;``. In the case of the regex matches, escape all double quotes ``\"`` and ensure the match doesn't end in an unescaped backslash ``\\``. The path must be unique among the paths of all routes of the VirtualServer. Check the [location](https://nginx.org/en/docs/http/ngx_http_core_module.html#location) directive for more information. | ``string`` | Yes |
...
Note that each subroute must have a `path` that starts with the same prefix (here `/coffee`) that the route of the VirtualServer defines. Additionally, the `host` in the VirtualServerRoute must be the same as the `host` of the VirtualServer.
...
5.4.1
/examples/ingress-resources/externalname-services#prerequisites) ). NGINX Ingress Controller tracks endpoint changes for upstream services in the same namespace and in other namespaces referenced with the ``namespace/service-name`` format. | ``string`` | Yes |
|``subselector`` | Selects the pods within the service using label keys and values. By default, it selects all pods of the service. Note: ensure the specified labels exist in the pods when you create them. If you update the pod labels, NGINX Ingress Controller will not see that change until you change the number of the pods. | ``map[string]string`` | No |
...
|``slow-start`` | The slow start allows an upstream server to gradually recover its weight from 0 to its nominal value after it recovers or becomes available or when the server becomes available after a period of time you considered it unavailable. By default, NGINX Plus disables slow start. See the [slow_start](https://nginx.org/en/docs/http/ngx_http_upstream_module.html#slow_start) parameter of the server directive. Note: You cannot use the parameter along with the ``random`` , ``hash`` or ``ip_hash`` load balancing methods and NGINX Plus will ignore it. | ``string`` | No |
|``queue`` | Configures a queue for an upstream. NGINX places a client request into the queue if NGINX cannot select an upstream server immediately while processing the request. By default, NGINX configures no queue. Note: this feature is supported only in NGINX Plus. | [queue](#upstreamqueue) | No |
...
The queue field configures a queue. NGINX places a client request into the queue if NGINX cannot select an upstream server immediately while processing the request:
...
|``fails`` | The number of consecutive failed health checks of a particular upstream server after which NGINX Plus considers this server unhealthy. The default is ``1``. | ``integer`` | No |
|``passes`` | The number of consecutive passed health checks of a particular upstream server after which NGINX Plus considers the server healthy. The default is ``1``. | ``integer`` | No |
...
|``grpcService`` | The gRPC service that NGINX Plus monitors on the upstream server. Only valid on gRPC type upstreams. | ``string`` | No |
...
|``persistent`` | Set the initial “up” state for a server after reload if you considered the server healthy before reload. Enabling persistent requires that you also set the mandatory parameter to `true`. | ``bool`` | No |
...
The SessionCookie field configures session persistence which allows requests from the same client to go to the same upstream server. NGINX passes information about the designated upstream server in a session cookie that NGINX generates.
...
|``path`` | The path that NGINX uses to set the cookie. | ``string`` | No |
...
|``domain`` | The domain that NGINX uses to set the cookie. | ``string`` | No |
...
In the example below, the client passes requests to an upstream `coffee`:
...
In the example below, the client passes requests to a url `http://www.nginx.com`:
...
|Field | Description | Type | Required |
| ---| ---| ---| --- |
|``upstream`` | The name of the upstream to which NGINX proxies the requests. Define the upstream with that name in the resource. | ``string`` | Yes |
...
|Field | Description | Type | Required |
| ---| ---| ---| --- |
|``hide`` | The headers that NGINX does not pass* in the response to the client from a proxied upstream server. See the [proxy_hide_header](http://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_hide_header) directive for more information. | ``[]string`` | No |
