...
| *nginx.org/upstream-vhost* | N/A | Sets the `Host` header sent to the upstream to a fixed hostname, using the [proxy_set_header](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_set_header) directive (or [grpc_set_header](https://nginx.org/en/docs/http/ngx_http_grpc_module.html#grpc_set_header) for gRPC backends). When not set, the upstream receives the Host header from the original request. The value must be a lowercase DNS (RFC 1123) subdomain. NGINX variables and ports are not supported. In Master and Minion (Mergeable) Ingresses, this annotation can be set on the Master and is inherited by its Minions. A Minion that sets its own value overrides the inherited one. | N/A | *example.internal* [upstream-vhost](https://github.com/nginx/kubernetes-ingress/tree/v{{< nic-version >}}/examples/ingress-resources/upstream-vhost) |
