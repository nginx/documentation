...
| *ingress.kubernetes.io/ssl-redirect* (DEPRECATED) | *ssl-redirect* | Sets a redirect rule for all incoming HTTP traffic to force incoming traffic over HTTPS when TLS is configured. The redirect code can be configured with the `nginx.org/http-redirect-code` annotation or the `http-redirect-code` ConfigMap key. *Note: This annotation is deprecated. Use `nginx.org/ssl-redirect` instead.* | *True* |  |
