...
| *nginx.org/policies* | N/A | Applies one or more [Policy resources]({{< ref "/nic/configuration/policy-resource/before-you-begin.md" >}}) to an Ingress. Specify a comma-separated list of policy names. | N/A | *webapp-policy* |
| *nginx.com/policies* | N/A | Applies one or more [Policy resources]({{< ref "/nic/configuration/policy-resource/before-you-begin.md" >}}) to an Ingress enabling NGINX Plus use-cases. Specify a comma-separated list of policy names. | N/A | *waf-policy* |
...
| *nginx.org/proxy-http-version* | N/A | Sets the [proxy_http_version](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_http_version) directive for connections to the upstream servers. Supported values are `1.0`, `1.1`, and `2`. If this annotation is unset and the backend Service port defines `appProtocol: kubernetes.io/h2c`, NGINX Ingress Controller uses HTTP/2 (`2`) automatically. An explicit annotation value takes precedence over the inferred value. The setting is ignored for gRPC services, which always use HTTP/2. WebSocket services require HTTP/1.1. Minion Ingresses inherit this annotation on mergeable Ingresses. *Requires NGINX >= v1.29.4 for the value `2`*. | N/A | [proxy-http-version](https://github.com/nginx/kubernetes-ingress/tree/v{{< nic-version >}}/examples/ingress-resources/proxy-http-version) |
