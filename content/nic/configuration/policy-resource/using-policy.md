...
f5-description: How to create, apply, and validate Policy resources with kubectl.
f5-summary: >
  Shows how to manage Policy resources with standard kubectl commands.
  Covers attaching policies to VirtualServer, VirtualServerRoute, and Ingress resources, how NGINX Ingress Controller handles invalid policies, and the structural and comprehensive validation it performs.
f5-keywords: "nginx ingress controller, nic, kubectl, policy resource, validation, invalid policy"
...
If a policy is invalid, the VirtualServer or VirtualServerRoute gets the [status]({{< ref "/nic/configuration/global-configuration/reporting-resources-status.md#virtualserver-and-virtualserverroute-resources" >}}) state `Warning`, with a message that explains why the policy is invalid.
...
If a resource passes structural validation, NGINX Ingress Controller's comprehensive validation runs next.

### Comprehensive validation

NGINX Ingress Controller validates the fields of a Policy resource. If a resource is invalid, NGINX Ingress Controller rejects it. The resource continues to exist in the cluster, but NGINX Ingress Controller ignores it.

Use `kubectl` to check whether NGINX Ingress Controller successfully applied a Policy configuration. For the example `webapp-policy` Policy, run:
...
If you create an invalid resource, NGINX Ingress Controller rejects it and emits a Rejected event. For example, if you create a Policy `webapp-policy` with an invalid IP `10.0.0.` in the `allow` field, you get:
...
If you make an existing resource invalid, NGINX Ingress Controller rejects it.
