...
|*use-forwarded-headers* | Preserves the client's incoming `X-Forwarded-Host`, `X-Forwarded-Port`, and `X-Forwarded-Proto` headers and passes them to backend services, instead of overwriting them with the Ingress Controller's own connection details. Use this when the Ingress Controller runs behind an external load balancer or CDN that already sets these headers. Mutually exclusive with `disable-forwarded-headers`. Not applicable to VirtualServer or VirtualServerRoute. | *false* |
