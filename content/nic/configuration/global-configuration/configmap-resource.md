...
|*use-forwarded-headers* | Preserves the client's incoming `X-Forwarded-Host`, `X-Forwarded-Port`, and `X-Forwarded-Proto` headers and passes them to backend services, instead of overwriting them with values set by NGINX Ingress Controller. Use this when NGINX Ingress Controller runs behind an external load balancer or CDN that already sets these headers. This setting does not affect `X-Forwarded-For`, which NGINX Ingress Controller always passes through, with the client's address appended. Mutually exclusive with `disable-forwarded-headers`. Not applicable to VirtualServer or VirtualServerRoute. | *false* |
