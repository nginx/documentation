...
- _Additional load balancing methods_: The `random two least_time` method and its derivatives become available. The `least_time` method is available on NGINX Open Source in NGINX Ingress Controller 5.7.0 and later. The NGINX [`ngx_http_upstream_module` documentation](https://nginx.org/en/docs/http/ngx_http_upstream_module.html) has the complete list of load balancing methods.
