...
F5 WAF for NGINX inspects client-facing requests. Internal subrequests generated by NGINX modules are not inspected. This applies to, for example:
...
### Example: demonstrate subrequest behavior with njs
The example here demonstrates the general rule: only client-facing requests are inspected by F5 WAF for NGINX; internal subrequests triggered by modules are not inspected.
...
Example of the njs handler issuing an internal subrequest to `/<script>`.
```shell
curl "localhost/"  
```
This internal subrequest is not inspected by App Protect and succeeds. Example output:
```shell
{"uri":"/<script>","code":200,"body":"Hello! I got your URI request - /foo//\n"}
```
Example of a direct, client-facing request that is inspected by App Protect and is blocked according to your security policy.
```shell
curl "localhost/<script>"
```
This direct request is blocked. Example output:
```shell
