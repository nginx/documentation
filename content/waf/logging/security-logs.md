...
| ------- | ------- | ----------- | ------- |
|request_type | Log according to what F5 WAF for NGINX detected in the request. | Enumerated values:<ul><li>**all:** all requests, both legal and illegal.</li><li>**illegal:** requests with violations (that is, either alerted or blocked).</li><li>**blocked:** requests with violations that were blocked.</li></ul> | all |
...
| policy_name | The name of the F5 WAF for NGINX policy for which the violation was triggered. | default, grpc |
| protocol | The protocol used, either HTTP or HTTPS if terminating SSL on F5 WAF for NGINX. | default, grpc |
