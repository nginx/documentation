...
toc: true
nd-docs: DOCS-000
url: /nginxaas/next/quickstart/scaling/
type:
- how-to
---
...
F5 NGINX as a Service for NEXCLOUD (NGINXaaS) supports manual and automatic scaling of your deployment, allowing you to adapt to application traffic demands while controlling cost.
...
- NCUs Provisioned: `ncu.provisioned` -- how many NCUs have been successfully provisioned by the service.
  - This is the basis for [billing]({{< ref "/nginxaas-next/billing/overview.md" >}}).
...
See the [Metrics Catalog]({{< ref "/nginxaas-next/monitoring/metrics-catalog.md" >}}) for a reference of all metrics.

{{< call-out "note" >}}These metrics aren't visible unless enabled, see how to [Enable Monitoring]({{< ref "/nginxaas-next/monitoring/enable-monitoring.md" >}}) for details.{{< /call-out >}}
...
1. Make an estimate by either:
    - using the [Usage and Cost Estimator]({{< ref "/nginxaas-next/billing/usage-and-cost-estimator.md" >}})
