...
toc: true
nd-docs: DOCS-000
url: /nginxaas/next/quickstart/runtime-state-sharing/
type:
- how-to
---
...
F5 NGINX as a Service for NEXCLOUD (NGINXaaS) supports runtime state sharing using the [Zone Synchronization module](https://nginx.org/en/docs/stream/ngx_stream_zone_sync_module.html) to synchronize shared memory zones across NGINXaaS instances.
...
For information on enabling synchronization for rate limiting with NGINXaaS for NEXCLOUD, please visit the [Rate Limiting]({{< ref "/nginxaas-next/quickstart/rate-limiting.md" >}}) documentation.

## Configuring runtime state sharing among NGINXaaS for NEXCLOUD deployment cluster instances
...
{{< call-out "note" >}}To enhance security, set up security rules for both incoming and outgoing traffic in the virtual network linked to the NSG of the subnet hosting NGINXaaS for NEXCLOUD deployment. These rules should limit TCP traffic to the `zone_sync_server` port.{{< /call-out >}}

## Enable the SSL/TLS protocol for connections to another cluster instance of the NGINXaaS for NEXCLOUD deployment
...
## Enable verification of certificate of another cluster instance of the NGINXaaS for NEXCLOUD deployment
...
## Set up certificate-based authentication across cluster instances of the NGINXaaS for NEXCLOUD deployment
