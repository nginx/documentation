...
description: "Learn how to set up OAuth2 client credentials for programmatic access to the F5 Application Delivery Service for Google Cloud API."
weight: 100
toc: true
url: /application-delivery-service/google/quickstart/security-controls/client-credentials/
...
f5-summary: >
    ...
    them for an access token that authenticates requests to the F5 Application Delivery Service for Google Cloud API.
...
This guide explains how to create and use client credentials for automating access to F5 Application Delivery Service for Google Cloud APIs. Client credentials enable automation tools such as CI/CD pipelines to manage certain F5 Application Delivery Service for Google Cloud resources without requiring user login.

To authenticate, you exchange your client credentials (client ID and secret) for a short-lived access token from the F5 Application Delivery Service for Google Cloud token endpoint. This access token is then used in the Authorization header of your API requests. Access tokens have limited validity, after which you'll need to request a new one using the same credentials.

Client credentials are scoped to an organization and expire after a set period (up to 1 year, 6 months recommended).

## Before you begin

- You must be logged in to the [F5 Application Delivery Service Console](https://console.nginxaas.net/).

## Create client credentials

Follow these steps to create a new client credential through the F5 Application Delivery Service for Google Cloud console:

1. Log in to [F5 Application Delivery Service Console](https://console.nginxaas.net/).
...
- [Certificates]({{< ref "/ads/google/deploy/ssl-tls-certificates/overview.md" >}})
- [Configs]({{< ref "/ads/google/deploy/nginx-configuration/configuration-rules.md" >}})
- [Deployments]({{< ref "/ads/google/deploy/create-deployment/deploy-console.md" >}})

### Client limits

Organizations are limited to a maximum of 10 client credentials. To request an increase to this limit, contact the [NGINX Support]({{< ref "/ads/support.md" >}}).
...
| Client limit per organization | 10 clients contact [NGINX Support]({{< ref "/ads/support.md" >}}) to increase |
...
[Manage your F5 Application Delivery Service for Google Cloud certificates]({{< ref "/ads/google/deploy/ssl-tls-certificates/overview.md" >}})

[Manage your NGINX configurations]({{< ref "/ads/google/deploy/nginx-configuration/configuration-rules.md" >}})

[Manage your F5 Application Delivery Service for Google Cloud deployments]({{< ref "/ads/google/deploy/create-deployment/deploy-console.md" >}})