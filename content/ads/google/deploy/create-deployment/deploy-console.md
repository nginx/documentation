...
toc: true
f5-docs: DOCS-000
url: /application-delivery-service/google/deploy/create-deployment/deploy-console/
...
This guide explains how to deploy F5 Application Delivery Service for Google Cloud using [Google Cloud Console](https://console.cloud.google.com) and the F5 Application Delivery Service Console. The deployment process involves creating a new deployment, configuring the deployment, and testing the deployment.

## Before you begin

Before you can deploy F5 Application Delivery Service for Google Cloud, follow the steps in the [Prerequisites]({{< ref "/ads/google/deploy/prerequisites/" >}}) topic to subscribe to the F5 Application Delivery Service for Google Cloud offering in the Google Cloud Marketplace.

### Create a network attachment

F5 Application Delivery Service for Google Cloud requires a [network attachment](https://cloud.google.com/vpc/docs/about-network-attachments) to connect your F5 Application Delivery Service for Google Cloud deployment to your VPC network. The network attachment must be created in a region we support.
...
1. Make a note of the network attachment ID as it will be needed in the next steps to create your F5 Application Delivery Service for Google Cloud deployment. You can find the network attachment ID in the Google Cloud Console by following the steps below:
...
Once you have completed the subscription process and created a network attachment, you can access the F5 Application Delivery Service Console.

{{< include "/ads/access-console.md" >}}

{{< call-out class="caution" >}}
Select the Geography that supports the region where you created your network attachment.
See [F5 Application Delivery Service for Google Cloud supported regions]({{< ref "/ads/google/overview.md#supported-regions" >}}).
{{< /call-out >}}

## Create or import an NGINX configuration

{{< include "/ads/create-or-import-nginx-config.md" >}}

## Create a new deployment

Next, create a new F5 Application Delivery Service for Google Cloud deployment using the F5 Application Delivery Service Console:
...
1. Select {{< icon "plus" >}} **Add Deployment** to create a new deployment.
   ...
   - Select **Managed Public Endpoint** or **Private Endpoint** under Service Frontend. 
      - Refer to the [Service Frontend]({{< ref "/ads/google/overview.md#service-frontend" >}}) documentation for more information on these two frontend types.
...
{{< call-out class="important" >}}If the **Connection preference** on the Network Attachment resource is set to **Accept connections from selected projects**, you will need to add the **F5 Application Delivery Service for Google Cloud deployment project** to the list of **Accepted projects** for the deployment to provision successfully. The F5 Application Delivery Service for Google Cloud deployment `Project ID` can be found under the `Cloud Info` section for your deployment. Failing to do so will leave the deployment in a `Pending` state, with details provided on the necessary actions required to proceed.{{< /call-out >}}
...
1. Select **Edit** to modify the deployment description, NCU Capacity, and WAF enablement.
   - You can also configure monitoring from here. Detailed instructions can be found in [Enable Monitoring]({{< ref "/ads/google/monitoring/enable-monitoring.md" >}})
...
1. To modify the contents of the NGINX configuration, see [Update an NGINX Configuration]({{< ref "/ads/google/deploy/nginx-configuration/nginx-configuration-console.md#update-an-nginx-configuration" >}}).
...
To set up private connectivity to your F5 Application Delivery Service for Google Cloud deployment, create a [Private Service Connect (PSC) endpoint](https://docs.cloud.google.com/vpc/docs/configure-private-service-connect-services) in the same VPC as your internal clients.

1. Go to the [Google Cloud Console](https://console.cloud.google.com/) and select the project where you want to create networking resources for your F5 Application Delivery Service for Google Cloud deployment.
...
1. Create a PSC endpoint. See [Google's documentation on creating an endpoint](https://docs.cloud.google.com/vpc/docs/configure-private-service-connect-services#create-endpoint) for a step-by-step guide.
    - For **Target service**, enter your F5 Application Delivery Service for Google Cloud deployment's Service Attachment, which is visible on the `Deployment Details` section for your deployment.


### External traffic

To set up public connectivity for external clients, configure a [Private Service Connect (PSC) backend](https://cloud.google.com/vpc/docs/private-service-connect-backends) for your F5 Application Delivery Service for Google Cloud deployment.
...
{{< details summary="Show helper script" >}}
   ...
   # - A valid Service Attachment URI from F5 Application Delivery Service for Google Cloud
...
1. To test your deployment, connect to the IP address created in [Set up connectivity]({{< ref "/ads/google/deploy/create-deployment/deploy-console.md#set-up-connectivity-private-endpoint-only" >}}) or the service endpoint created with your managed public endpoint deployment.
...
- [Monitor your deployment]({{< ref "/ads/google/monitoring/enable-monitoring.md" >}})
- [Manage certificates in Google Secret Manager]({{< ref "/ads/google/deploy/ssl-tls-certificates/ssl-tls-certificates-secret-manager.md" >}})
