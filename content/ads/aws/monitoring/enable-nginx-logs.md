...
toc: true
f5-docs: DOCS-000
url: /application-delivery-service/aws/monitoring/enable-nginx-logs/
...
f5-summary: >
  ...
  This guide details IAM permission setup, log export toggles in the F5 Application Delivery Service for AWS console, and querying log streams using CloudWatch Logs Insights.
...
When you enable log export, F5 Application Delivery Service for AWS writes NGINX logs to a CloudWatch Logs log group (the **Log Group**) in a log stream named `logs`.

## Before you begin

- Configure a **Role ARN** in the **Identity** section of your deployment. F5 Application Delivery Service for AWS uses this IAM role to export logs to CloudWatch. See [Identity and access management]({{< ref "/ads/aws/deploy/access-management.md" >}}) for how to create the role and attach the [Policy for CloudWatch Logs]({{< ref "/ads/aws/deploy/access-management.md#step-3-add-inline-policies-to-your-role" >}}).
- Grant the IAM role the permissions required to export logs:
  - `logs:CreateLogStream` and `logs:PutLogEvents`: required for F5 Application Delivery Service for AWS to write logs to the log group.
...
{{< include "/ads/logging-config-error-logs.md" >}}

## Set up access logs

{{< include "/ads/logging-config-access-logs.md" >}}
...
1. In the **Log Group Name** field, enter the name of the CloudWatch Logs log group you want to receive exported logs. If you leave this field at its default, F5 Application Delivery Service for AWS uses a log group named after the deployment object ID.
1. Select **Update**.
...
To stop exporting logs, update your F5 Application Delivery Service for AWS deployment to turn off the log export toggle. To update the deployment, in the F5 Application Delivery Service for AWS console,
...
If Amazon CloudWatch Logs is not showing any logs, check for **Failed Log Export to CloudWatch** events from your F5 Application Delivery Service for AWS deployment.
...
- [Enable monitoring]({{< ref "/ads/aws/monitoring/enable-monitoring.md" >}})
