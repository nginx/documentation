...
They enable use cases not supported with the Ingress resource, such as traffic splitting and advanced content-based routing. NGINX Ingress Controller implements the resources as [Custom Resources](https://kubernetes.io/docs/concepts/extend-kubernetes/api-extension/custom-resources/).
...
|Field | Description | Type | Required |
| ---| ---| ---| --- |
|``issuer`` |  the name of an Issuer. An Issuer is a cert-manager resource which describes the certificate authority capable of signing certificates. The Issuer must be in the same namespace as the VirtualServer resource. Please note that you must specify one of `issuer` and `cluster-issuer`, but not both. | ``string`` | No |
|``cluster-issuer`` | the name of a ClusterIssuer. A ClusterIssuer is a cert-manager resource which describes the certificate authority capable of signing certificates. It does not matter which namespace your VirtualServer resides, as ClusterIssuers are non-namespaced resources. Please note that you must specify one of `issuer` and `cluster-issuer`, but not both. | ``string`` | No |
|``issuer-kind`` | The kind of the external issuer resource, for example AWSPCAIssuer. This is only necessary for out-of-tree issuers. You cannot define this if you also define `cluster-issuer`. | ``string`` | No |
|``issuer-group`` | The API group of the external issuer controller, for example awspca.cert-manager.io. This is only necessary for out-of-tree issuers. You cannot define this if you also define `cluster-issuer`. | ``string`` | No |
|``common-name`` | This field allows you to configure spec.commonName for the Certificate that cert-manager generates. This configuration adds a CN to the x509 certificate. | ``string`` | No |
|``duration`` | This field allows you to configure the spec.duration field for the Certificate that cert-manager generates. Specify the value using a [Go time.Duration](https://pkg.go.dev/time#ParseDuration) string format, which does not allow the d (days) suffix. You must specify these values using s, m, and h suffixes instead. | ``string`` | No |
|``renew-before`` |  this annotation allows you to configure the spec.renewBefore field for the Certificate that cert-manager generates. Specify the value using a [Go time.Duration](https://pkg.go.dev/time#ParseDuration) string format, which does not allow the d (days) suffix. You must specify these values using s, m, and h suffixes instead. | ``string`` | No |
|``usages`` |  This field allows you to configure the spec.usages field for the Certificate that cert-manager generates. Pass a string with comma-separated values i.e. ``key agreement,digital signature, server auth``. The [cert-manager API documentation](https://cert-manager.io/docs/reference/api-docs/#cert-manager.io/v1.KeyUsage) provides an exhaustive list of supported key usages. | ``string`` | No |
...
|``recordType`` | The record Type to create, e.g. "A", "AAAA", "CNAME". This is automatically computed based on the external endpoints if not defined. | ``string`` | No |
...
|``matches`` | The matching rules for advanced content-based routing. Requires the default ``action`` or ``splits``.  The default ``action`` or ``splits`` handle unmatched requests. | [matches](#match) | No |
...
|``routeSelector`` | The RouteSelector allows selecting VirtualServerRoute resources using label selectors instead of specifying them by name and namespace. Use RouteSelector to decouple VirtualServerRoute updates from the VirtualServer so you can add, remove, or update matching VirtualServerRoutes without changing the VirtualServer. | [routeSelector](#virtualserverrouterouteselector) | No |
...
Use RouteSelector when you want to add, remove, or update VirtualServerRoute resources without editing the VirtualServer. To do that, make sure:

- The VirtualServer route `path` matches the prefix rules for the VirtualServerRoute subroutes.
- The VirtualServerRoute has labels that match the VirtualServer `routeSelector`.
- The VirtualServerRoute `spec.host` matches the VirtualServer `spec.host`.
...
|Field | Description | Type | Required |
| ---| ---| ---| --- |
|``matchLabels`` | A map of key-value pairs. Each key-value pair in the matchLabels map is equivalent to an element of matchExpressions, whose key field is "key", the operator is "In", and the values array contains only "value". AND all requirements. | ``map[string]string`` | Yes |
|``matchExpressions`` | A list of label selector requirements. AND all requirements. For more information on label selector requirements, see the [Kubernetes documentation](https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/#resources-that-support-set-based-requirements). | N/A | No |
...
|``subselector`` | Selects the pods within the service using label keys and values. By default, it selects all pods of the service. Note: expect the specified labels to be present in the pods when you create them. If you update the pod labels, NGINX Ingress Controller will not see that change until the number of the pods changes. | ``map[string]string`` | No |
...
|``next-upstream`` | Specifies in which cases NGINX should pass a request to the next upstream server. See the [proxy_next_upstream](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_next_upstream) directive. The default is ``error timeout``. | ``string`` | No |
|``next-upstream-timeout`` | The time during which NGINX can pass a request to the next upstream server. See the [proxy_next_upstream_timeout](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_next_upstream_timeout) directive. The ``0`` value turns off the time limit. The default is ``0``. | ``string`` | No |
...
|``slow-start`` | The slow start allows an upstream server to gradually recover its weight from 0 to its nominal value after it recovers or becomes available or when the server becomes available after a period of time NGINX considered it unavailable. By default, NGINX disables the slow start. See the [slow_start](https://nginx.org/en/docs/http/ngx_http_upstream_module.html#slow_start) parameter of the server directive. Note: You cannot use the parameter along with the ``random`` , ``hash`` or ``ip_hash`` load balancing methods and NGINX will ignore it. | ``string`` | No |
|``queue`` | Configures a queue for an upstream. Place a client request into the queue if NGINX cannot select an upstream server immediately while processing the request. By default, NGINX does not configure a queue. Note: this feature is supported only in NGINX Plus. | [queue](#upstreamqueue) | No |
...
|Field | Description | Type | Required |
| ---| ---| ---| --- |
|``enable`` | Enables HTTPS for requests to upstream servers. The default is ``False`` , meaning that NGINX will use HTTP. Note: by default, NGINX will not verify the upstream server certificate. To enable the verification, configure an [EgressMTLS Policy](https://docs.nginx.com/nic/configuration/policy-resource/#egressmtls). | ``boolean`` | No |

### Upstream.Queue

The queue field configures a queue. Place a client request into the queue if the system cannot select an upstream server immediately while processing the request:
...
|``timeout`` | The timeout of the queue. A request cannot stay in the queue longer than the timeout. The default is ``60s``. | ``string`` | No |
...
|``fails`` | The number of consecutive failed health checks of a particular upstream server after which NGINX considers this server unhealthy. The default is ``1``. | ``integer`` | No |
|``passes`` | The number of consecutive passed health checks of a particular upstream server after which NGINX considers the server healthy. The default is ``1``. | ``integer`` | No |
|``port`` | The port for health check requests. By default, NGINX uses the [server port](https://nginx.org/en/docs/http/ngx_http_upstream_hc_module.html#health_check_port). Note: in contrast with the port of the upstream, this port is not a service port, but a port of a pod. | ``integer`` | No |
|``tls`` | The TLS configuration for health check requests. By default, NGINX uses the ``tls`` field of the upstream. | [upstream.tls](#upstreamtls) | No |
|``connect-timeout`` | The timeout for establishing a connection with an upstream server. By default, NGINX uses the ``connect-timeout`` of the upstream. | ``string`` | No |
|``read-timeout`` | The timeout for reading a response from an upstream server. By default, NGINX uses the ``read-timeout`` of the upstream. | ``string`` | No |
|``send-timeout`` | The timeout for transmitting a request to an upstream server. By default, NGINX uses the ``send-timeout`` of the upstream. | ``string`` | No |
...
|``grpcService`` | The gRPC service that NGINX monitors on the upstream server. Only valid on gRPC type upstreams. | ``string`` | No |
|``mandatory`` | Require every newly added server to pass all configured health checks before NGINX Plus sends traffic to it. If you do not specify this, or set it to false, NGINX Plus will initially consider the server healthy. When combined with [slow-start](https://nginx.org/en/docs/http/ngx_http_upstream_module.html#slow_start), it gives a new server more time to connect to databases and “warm up” before it handles its full share of traffic. | ``bool`` | No |
|``persistent`` | Set the initial “up” state for a server after reload if NGINX considered the server healthy before reload. Enabling persistent requires that you also set the mandatory parameter to `true`. | ``bool`` | No |
|``keepalive-time`` | Enables [keepalive](https://nginx.org/en/docs/http/ngx_http_upstream_module.html#keepalive) connections for health checks and specifies the time during which NGINX can process requests through one keepalive connection. The default is ``60s``. | ``string`` | No |

### Upstream.SessionCookie

The SessionCookie field configures session persistence which allows requests from the same client to go to the same upstream server. NGINX passes information about the designated upstream server in a session cookie that it generates.
...
|Field | Description | Type | Required |
| ---| ---| ---| --- |
|``hide`` | The headers that NGINX will not pass* in the response to the client from a proxied upstream server. See the [proxy_hide_header](http://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_hide_header) directive for more information. | ``[]string`` | No |
